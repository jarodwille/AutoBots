...
def load_model(args, model_config, k_attr, num_other_agents, pred_horizon, map_attr, num_agent_types, predict_yaw):
    if torch.cuda.is_available() and not args.disable_cuda:
        device = torch.device("cuda")
    else:
        device = torch.device("cpu")
    
    autobot_model = AutoBotJoint(k_attr=k_attr,
                                 ...
                                 predict_yaw=predict_yaw).to(device)
...
if __name__ == "__main__":
    ...
    autobot_model, device = load_model(args, model_config, val_dset.k_attr, val_dset.num_others, val_dset.pred_horizon,
                                       val_dset.map_attr, val_dset.num_agent_types, val_dset.predict_yaw)
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader):
            ...
            # all of these are in frame of ego vehicle
            ego_in, ego_out, agents_in, agents_out, roads, agent_types, extras, large_roads = data
            ego_in = ego_in.float().to(device)
            ego_out = ego_out.float().to(device)
            agents_in = agents_in.float().to(device)
            agents_out = agents_out.float().to(device)
            roads = roads.float().to(device)
            agent_types = agent_types.float().to(device)

            # Process extras
            translation = extras[0].cpu().numpy() # translation of ego vehicle
            rotation = extras[1].cpu().numpy() # rotation of ego vehicle
            instance_tokens = list(extras[2]) # id of the ego agent
            sample_tokens = list(extras[3]) # id of the frame of a scene
            agents_tokens = np.array(extras[4]).T.tolist() # id of non-ego agents in scene. should be list of lists?
            
            # TODO: REWRITE THIS STUFF
            pred_obs, mode_preds = autobot_model(ego_in, agents_in, roads, agent_types)
            # pred_seqs now has sequences for M agents (index 3)
            pred_seqs = pred_obs[:, :, :, :, :2].cpu().numpy().transpose((2, 0, 1, 3, 4))
            mode_preds = mode_preds.cpu().numpy()
        
            for b in range(min(args.batch_size, len(pred_seqs))):
                # loop over ego and non-ego agents
                if "None" in agents_tokens[b]:
                    none_idx = agents_tokens[b].index("None")
                else:
                    none_idx = 20
                for agent_i in range(none_idx + 1):
                    curr_out = {}
                    if agent_i == 0:
                        curr_out["instance"] = instance_tokens[b]
                    else:
                        curr_out["instance"] = agents_tokens[b][agent_i-1]
                    curr_out["sample"] = sample_tokens[b]
                    mode_preds[b] = recompute_probs(pred_seqs[b, :, :, agent_i], mode_preds[b])
                    curr_out["probabilities"] = mode_preds[b].tolist()
                    curr_out["prediction"] = []
                    for k in range(model_config.num_modes):
                        pred_seq = pred_seqs[b, k, :, agent_i]
